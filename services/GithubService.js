...
class GithibService {
  ...
  async fetchProjects(getExtras = true) {
    let repos = await Promise.all(
      Project.map(async (proj) => {
        if (proj.include.length > 0) {
          let includedRepo = await Promise.all(
            proj.include.map(async (name) => {
              const { data } = await this.axios.get(
                `${this.baseURL}/repos/${proj.orgsName}/${name}`
              );
              return data;
            })
          );
          if (proj.exclude.length > 0) {
            includedRepo = includedRepo.filter(
              (repo) => !proj.exclude.includes(repo.name)
            );
          }
          return includedRepo;
        } else {
          let { data } = await this.axios.get(
            `${this.baseURL}/orgs/${proj.orgsName}/repos`
          );
          data = data
            .filter((repo) => repo.fork === false)
            .filter((data) => data.topics.length)
            .sort((a, b) =>
    ...
    if (getExtras) {
      let repos = await Promise.all(
        repositories.map(async (repo, i) => {
          const data = await this.fetchContributors(
            repo.owner.login,
            repo.name
          );
          repo['contributors'] = data;
          repo['top_contributors'] = [...data]
            .sort((a, b) => b.contributions - a.contributions)
            .slice(0, 3);
          const lang = await this.fetchLanguages(repo.languages_url);
          repo['languages'] = lang;
          repo['icon_url'] = `https://ui-avatars.com/api/?background=${
            COLORS[i % 3].bg
          }&color=${COLORS[i % 3].fg}&name=${repo.name}&size=256&font-size=0.4`;
          return repo;
        })
      );
      return repos;
  ...
  async getHomePageData() {
    ...
    const [repos, topFive] = await Promise.all([
      this.fetchFeaturedProjects(data),
      this.fetchTopFiveContributors(data),
    ]);
  ...
  async fetchFeaturedProjects(projects) {
    ...
    const repositories = projects.filter(
      (repo) =>
        ...
        repo.topics.includes('contributions-welcome') ||
        Project.map((proj) => proj.include.includes(repo.name))
    );
    const repos = await Promise.all(
      repositories.map(async (repo, i) => {
        ...
        this.fetchLanguages(repo.languages_url).then((lang) => {
          repo['languages'] = lang;
        });
        repo['icon'] = {
        ...
        repo['icon_url'] = `https://ui-avatars.com/api/?background=${
          COLORS[i % 3].bg
        }&color=${COLORS[i % 3].fg}&name=${repo.name}&size=256&font-size=0.33`;
        return repo;
      })
    );
    return repos;
  ...
  async fetchTopFiveContributors(projects) {
    ...
    if (excludeTopContributors.length > 0) {
      contributors = contributors.filter(
        (user) => !excludeTopContributors.includes(user.login)
      );
    }
    contributors = contributors.slice(0, 5);

    let topFive = await Promise.all(
  ...
  async readHomePageData() {
    try {
      const readFile = fs.readFileSync(
        path.join(process.cwd(), `cache/gitHub.json`),
        'utf-8'
      );
