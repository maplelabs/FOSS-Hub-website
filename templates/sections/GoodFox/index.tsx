...
export default function GoodFox({
  id,
  title,
  columnHeaders,
  listItems,
      ...
      <div className='uk-section uk-dark' style={{ marginBottom: '-20px' }}>
        <div className='uk-container'>
          <div className='uk-flex uk-flex-between uk-margin-medium-bottom'>
            <div
              className={`uk-h1 uk-text-bolder uk-margin-small-bottom ${styles['color_midnight-blue']}`}
            >
              {title}
            </div>
          ...
          {tabularView ? (
            <div
              className='uk-child-width-1-4@m uk-text-left uk-grid uk-grid-small uk-grid-match'
              data-uk-grid
            >
              {listItems.map((project, index) => (
                <Card data={project} key={index}></Card>
              ))}
            </div>
          ) : (
            <Table
